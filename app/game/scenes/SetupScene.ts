...
export class SetupScene extends Phaser.Scene {
  ...
  private showGenderStep(): void {
    ...
    options.forEach((opt, i) => {
      ...
      zone.on("pointerdown", () => {
        ...
        this.time.delayedCall(200, () => this.showSuitStep());
      });
  ...
  private showNameStep(): void {
    ...
    this.instructionText.setText("キーボードで入力  Enter で決定");
    this.playerName = "";
    this.nameText.setVisible(true);
    this.nameText.setText("▌");

    // Touch: add a simple "OK" button
    const okBg = this.add.graphics().setDepth(10);
    okBg.fillStyle(0x2255aa, 0.9);
    okBg.fillRoundedRect(270, 400, 100, 40, 8);
    okBg.lineStyle(2, 0x66aaff);
    okBg.strokeRoundedRect(270, 400, 100, 40, 8);

    const okText = this.add.text(320, 420, "けってい", {
      fontSize: "14px",
      color: "#ffffff",
      fontFamily: "monospace",
    }).setOrigin(0.5).setDepth(11);

    const okZone = this.add.zone(320, 420, 100, 40)
      .setInteractive().setDepth(12).setOrigin(0.5);
